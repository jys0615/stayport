...
공급사 실패를 5xx로 접지 않는 이유는 이 API의 실패가 공급사 단위로 부분적이기 때문입니다.
응답 전체의 상태 코드로는 "A는 됐고 B만 죽었다"를 표현할 자리가 없습니다. 이 선택을 두고 오간
반론과 판단은 design.md §7에 있습니다. 클라이언트가 무엇을 보고 재시도할지는 아래
`suppliers[]` 표에 있습니다.
...
| 필드 | 규칙 |
|---|---|
| `failures` | 실패 유형별 개수. `AUTH`·`INVALID_REQUEST`·`PARSE_ERROR`는 재시도해도 같고, `RATE_LIMIT`·`SUPPLIER_ERROR`·`TIMEOUT`은 시간이 지나면 달라질 수 있습니다. 여섯 유형 모두 테스트로 고정돼 있습니다 |
