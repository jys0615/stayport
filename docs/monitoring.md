...
정의되지 않으므로 `failure{type=TIMEOUT}` 카운터로만 남습니다. 성공률은 `call{outcome}`
횟수의 비율로 계산합니다. 별도 지표로 따로 두면 같은 사실을 두 지표로 들고 있게 되어
어긋날 수 있습니다.
...
오르면 스레드 인질 상황입니다. 이때 원인은 ①입니다. 트래픽이 늘어난 것이 아니므로 대응은
스케일아웃이 아니라 해당 공급사 격리(호출 제한 하향 등)입니다.
...
5xx는 0인 상태가 바로 그 장애입니다. 그래서 본문 판정을 지표로 만들어 뒀습니다.
...
**⑤ 데이터 품질 문제** — `stayport.supplier.skipped`가 오르면 검색 결과가 줄어든 원인은
재고에 있지 않습니다. 매핑이나 형식 문제라는 신호입니다. 무엇을 버렸는지는
`GET /internal/quarantine`에 원본째 남아 있습니다.

## 경보를 단다면

이 저장소 범위에서는 경보 시스템을 붙이지 않았지만, 붙인다면 우선순위는 이렇습니다:

1. **공급사별 실패율** — `failure` 증가율 ÷ `call` 횟수. 유형별로 임계가 달라야 합니다:
   `AUTH`는 1건도 즉시(재시도해도 안 바뀌는 실패), `TIMEOUT`은 비율로(일시적일 수 있음)
2. **공급사별 지연 p95.** 호출 제한(3초)의 2/3 지점을 경계로 잡습니다. 제한에 닿기 전에
   알아야 대응할 시간이 생깁니다
...
- 레지스트리가 인메모리(`SimpleMeterRegistry`)라 앱을 재시작하면 값이 사라집니다. 운영이라면
  `micrometer-registry-prometheus` 의존성 하나로 스크레이프 대상이 됩니다. 여기서 붙이지 않은
  것은 수집기 없이는 검증할 수 없는 코드가 되기 때문입니다
