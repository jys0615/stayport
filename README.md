...
이 시스템의 핵심은 정상 경로가 아니라 실패 경로다. 시간으로 보면 이렇게 동작한다 —
B가 침묵하면 3초에 끊고, 어느 쪽이든 3.5초 안에 200으로 응답한다:

![장애 격리 타임라인 — 두 공급사를 동시에 조회하고, B가 3초 안에 응답하지 않으면 B는 TIMEOUT으로 처리한 채 3.5초 안에 200으로 응답한다](docs/images/failure-timeline.png)

흉내 서버에 고장 스위치가 있으니 직접 꺼뜨려 보면서 확인하는 것이 가장 빠르다.
...
포트와 어댑터 구조다. 코어가 포트(주황 소켓)를 선언하고, 어댑터가 바깥에서 꽂힌다:

![포트·어댑터 구조 — 애플리케이션 코어가 공급사 조회·매핑 저장·격리 저장 포트를 정의하고, Supplier A·B 어댑터와 DB 저장 어댑터가 각 포트에 꽂혀 Supplier A, Supplier B, DB로 나간다](docs/images/architecture-core.png)

읽는 축 세 가지 — ① 포트가 어댑터가 아니라 코어 쪽에 있다: 유스케이스는 "무엇이
필요한가"만 선언하고 공급사 사정은 어댑터에서 끝난다. A는 실패를 HTTP 상태로, B는
200 속 응답 코드로 말하지만 그 차이는 어댑터 밖으로 새지 않는다. ② 공급사 호출은
병렬이고 각각 3초 제한 — 한쪽 실패는 그 공급사의 상태로만 남는다. ③ DB에는 매핑과
격리 데이터만 있다 — 가격·객실 정보는 저장하지 않고 매번 실시간으로 조회한다.
이 경계들은 문서 약속이 아니라 ArchUnit 규칙 5개로 빌드에서 강제된다(`ArchitectureTest`).
