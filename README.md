...
흡수해서 클라이언트는 어느 공급사에서 온 상품인지와 무관하게 같은 형태의 결과를 받습니다.
...
이 시스템의 핵심은 정상 경로가 아니라 실패 경로입니다. 시간으로 보면 이렇게 동작합니다 —
B가 침묵하면 3초에 끊고, 어느 쪽이든 3.5초 안에 200으로 응답합니다:

![장애 격리 타임라인 — 두 공급사를 동시에 조회하고, B가 3초 안에 응답하지 않으면 B는 TIMEOUT으로 처리한 채 3.5초 안에 200으로 응답합니다](docs/images/failure-timeline.png)
...
분해는 버리는 대신 `dailyBreakdown` 선택 필드로 보존했습니다 — 요금 분해는 한 번 버리면
역산이 안 됩니다. 세금을 최상위 필드로 두는 안은 기각했습니다: B에서 영원히 null이 되는
필드는 세금을 표시하는 클라이언트를 조용히 틀리게 만듭니다.
...
로그로 구분할 수 없습니다. 값은 전부 `application.yml`에 있습니다.
...
실시간 조회합니다. 동기화는 기동 시 1회 + `POST /internal/sync` 수동 — 목록이 정적인데 주기를
정할 근거가 없어서 주기 스케줄러는 두지 않았습니다. 같은 공급사 상품이 항상 같은 내부 식별자를
갖는 것은 DB UNIQUE 제약으로 보장합니다. 객실 타입 코드는 숙소 안에서만 유일하므로 객실 매핑
키에는 숙소 코드가 함께 들어갑니다.

**50개 제한과 수천 숙소 스케일**
공급사가 한 번에 받는 숙소 코드는 50개라 어댑터가 목록을 나눠 병렬로 부르고, 일부 묶음만
실패하면 받은 것은 버리지 않고 `PARTIAL`로 표시합니다. 숙소가 수천 개가 되면 실시간 전량
조회 자체가 예산을 넘으므로 사전 집계 캐시로 전환해야 합니다 — 로드맵은 design.md §5에
있습니다.
...
읽는 축은 세 가지입니다 — ① 포트가 어댑터가 아니라 코어 쪽에 있습니다: 유스케이스는 "무엇이
필요한가"만 선언하고 공급사 사정은 어댑터에서 끝납니다. A는 실패를 HTTP 상태로, B는
200 속 응답 코드로 말하지만 그 차이는 어댑터 밖으로 새지 않습니다. ② 공급사 호출은
병렬이고 각각 3초 제한 — 한쪽 실패는 그 공급사의 상태로만 남습니다. ③ DB에는 매핑과
격리 데이터만 있습니다 — 가격·객실 정보는 저장하지 않고 매번 실시간으로 조회합니다.
이 경계들은 문서 약속이 아니라 ArchUnit 규칙 5개로 빌드에서 강제됩니다(`ArchitectureTest`).
