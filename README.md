...
```
[기동] 공급사별 숙소 목록을 한 번 읽어 (공급사 코드 ↔ 내부 식별자) 매핑을 저장한다.
       이후 갱신은 POST /internal/sync 로 손으로. 재고·요금은 저장하지 않는다.

[검색] GET /api/v1/stays/search?checkIn&checkOut&adults&children
       ① 매핑에서 물어볼 숙소 코드를 공급사별로 모아 50개씩 자른다
       ② 코드가 있는 공급사만 동시에 호출한다 (WebClient, 전체 예산 3.5초)
       ③ 응답을 표준 모델로 정규화하고 내부 식별자로 갈아탄다
       ④ 예산 안에 도착한 것만 병합한다. 못 받은 공급사는 결과를 버리지 않고
          suppliers[] 에 상태와 사유로 남긴다
```

| 🧪 테스트 | ⏱ 응답 예산 | 🧨 장애 재현 | 📈 부하 실측 |
|:---:|:---:|:---:|:---:|
| 64개 전부 green | 어떤 장애에도 **3.5초 안에 200** | 재현 시나리오 [8가지](#-장애를-직접-내보기) | [3,484 → 9.9 req/s](docs/load-test.md)<br>스레드 천장 증명 |
...
**⑥ 속도 제한 — 두 공급사가 다른 방식으로 같은 말을 합니다**
...
흉내 서버를 장애로 둔 채 애플리케이션을 처음 기동하면(또는 `data/`를 지우고 재기동) 매핑이 빈 채로
...
언제 예산이 깨지는지는 세어볼 수 있습니다. 한 번에 나가는 청크 수를 P라 하면 공급사가 300ms로
답할 때 약 `50 × P × 10`개까지 버팁니다. P를 16으로 잡으면 8,000개, 공급사가 1초로 느려지면
2,400개에서 3.5초를 넘깁니다. 한계를 쥐고 있는 쪽은 우리 숙소 수보다 공급사의 응답 시간입니다.
P가 지금 무엇이고 왜 이 값을 직접 정해야 하는지는 design.md §5에 있습니다 — 처음에 기본값을
16으로 적었다가 소켓을 세어 보고 틀린 것을 발견한 자리이기도 합니다.
...
응답의 필드마다 판단이 하나씩 붙어 있습니다. 무엇을 왜 이렇게 뒀는지 적었고, 해석 규칙 전체는
[docs/api.md](docs/api.md)에 있습니다.

| 필드 | 왜 이렇게 뒀나 |
|---|---|
| `stayId` · `roomTypeId` | 공급사 코드를 내보내지 않습니다. 공급사가 바뀌어도 클라이언트가 쓰던 값이 살아 있어야 하고, 같은 상품은 재검색해도 같은 값이 나옵니다 |
| `stayName` · `roomTypeName` | 동기화 시점의 스냅샷입니다. 표시 이름의 정본을 매핑으로 정했기 때문에, 검색 때 공급사가 준 이름은 쓰지 않습니다 |
| `maxOccupancy` | 객실 1실이 받는 인원입니다. 요청 인원과 비교할 기준이 없으면 클라이언트가 몇 실을 잡아야 하는지 계산할 수 없습니다 |
| `availableRooms` · `bookable` | 요청 기간 **전체**를 예약할 수 있는 수, 즉 `min(날짜별 잔여)`입니다. 하루라도 0이면 0입니다. **0이어도 목록에서 빼지 않습니다** — "그런 숙소가 없다"와 "있는데 만실이다"는 다른 사실입니다 |
| `supplier` | 같은 건물이 공급사별로 두 번 나올 수 있어서, 어디서 온 상품인지 없으면 중복으로 읽힙니다 |
| `price.totalAmount` · `currency` | 기간 전체 총액, 세금 포함. 두 공급사가 공통으로 줄 수 있는 유일한 형태라 이걸 공통 분모로 삼았습니다 |
| `price.dailyBreakdown` | 날짜별 분해는 주는 공급사만 채웁니다. 없는 것을 나눠서 만들어내지 않고 `null`로 둡니다 |
| `breakfastIncluded` | 같은 객실이라도 공급사마다 다릅니다. 이걸 빼면 조건이 다른 상품을 같은 가격으로 비교하게 됩니다 |
| `suppliers[]` | 어느 공급사가 어디까지 답했는지를 본문에 싣습니다. 상태 코드 하나로는 "A는 됐고 B만 죽었다"를 표현할 자리가 없습니다 |
...
| [docs/ai-usage.md](docs/ai-usage.md) | AI를 어디에 썼고 그 답을 어떻게 수용·수정·거부했는지 |
