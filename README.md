...
**상품의 단위 — 숙소 > 객실 타입 2단계**
공급사 둘 다 상품을 숙소와 객실 타입 두 단계로 표현하고, 101호·102호 같은 물리 객실은 주지
않습니다. 그래서 우리 표준도 두 단계까지만 잡았습니다. 검색 결과 한 건은 숙소 × 객실 타입 ×
검색 조건이고, `availableRooms`는 그 타입의 객실이 몇 개 남았는지를 뜻합니다. 조식은 요금에
딸린 정보라 재고·요금 조회에서만 옵니다. 같은 객실이라도 공급사마다 값이 다를 수 있어서
병합하지 않고 상품 속성(`breakfastIncluded`)으로 남겼습니다. 조식 포함 여부를 무시하고 가격만
비교하면 조건이 다른 상품을 비교하게 됩니다.
...
실패하면 받은 것은 버리지 않고 `PARTIAL`로 표시합니다. 언제 예산이 깨지는지는 세어볼 수
있습니다. 커넥션 풀이 공급사당 16개라 청크는 16개씩 나눠 나가고, 공급사가 300ms로 답하면
약 8,000개, 1초로 느려지면 약 2,400개에서 3.5초를 넘깁니다. 유도 과정과 그 지점에서의
